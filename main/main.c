...
{
    uint32_t *sram_start = (uint32_t *)0x3FC88000; // SRAM start address for ESP32-S3
    uint32_t sram_size = 0x20000;                  // 128 KB
    uint32_t *sram_end = sram_start + (sram_size / sizeof(uint32_t));
...
void app_main()
{
    read_sram_before_init(); // Ensures this is called first

    bootloader_clear_bss_section();
    bootloader_read_bootloader_header();
    // Other bootloader initialization code...
